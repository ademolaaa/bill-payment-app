...
const SETTINGS_LINKS = [
  ...
  { id: 'notifications', label: 'Notification Preferences', href: '/settings/notifications', icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg> },
  ...
  { id: 'terms', label: 'Terms of Use', href: 'https://kyvatron.com/terms-and-conditions', icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z" /></svg> },
  { id: 'privacy', label: 'Privacy Policy', href: 'https://kyvatron.com/privacy-policy', icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 11c0 3.517-1.009 6.799-2.753 9.571m-3.44-2.04l.054-.09A13.916 13.916 0 008 11a4 4 0 118 0c0 1.017-.07 2.019-.203 3m-2.118 6.844A21.88 21.88 0 0015.171 17m3.839 1.132c.645-2.266.99-4.659.99-7.132A8 8 0 008 4.07M3 15.364c.64-1.319 1-2.8 1-4.364 0-1.457.39-2.823 1.07-4" /></svg> },
];
    ...
    <div className="flex flex-col min-h-screen bg-slate-50 dark:bg-slate-950 relative pb-24">
      ...
      <div className="flex-grow p-4 mt-2">
        <section className="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          {SETTINGS_LINKS.map((link, index) => (
            <Link key={link.id} href={link.href} className={`flex items-center p-4 hover:bg-gray-50 transition-colors ${index !== SETTINGS_LINKS.length - 1 ? 'border-b border-gray-50' : ''}`}>
              ...
              <span className="text-[16px] font-medium text-gray-900 dark:text-slate-200 flex-grow">{link.label}</span>
        ...
        <section className="mt-8">
          ...
          <p className="text-xs text-gray-600 dark:text-slate-700 text-center mt-3 px-4">
